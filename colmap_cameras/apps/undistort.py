...
from colmap_cameras import model_selector, ResizedCamera
...
def main():
    ...
    arg_parser.add_argument("--input_camera", type=str, required=True,
                            help="Input camera model in colmap format")
    ...
    arg_parser.add_argument("--output_size", type=int, nargs='+', default=None,
                            help="Output width height. Defaults to input size.")
    ...
    arg_parser.add_argument("--resize_ratio", type=float, default=None, help="Resize input camera by this factor")

    args = arg_parser.parse_args()

    input_camera = model_selector(args.input_camera.split()[0],
                                  list(map(float, args.input_camera.split()[1:])))
    ...
    if args.save_lut is not None:
        ...
        src = os.path.join(os.path.dirname(__file__), '..', 'utils', 'lut_remapper.py')
